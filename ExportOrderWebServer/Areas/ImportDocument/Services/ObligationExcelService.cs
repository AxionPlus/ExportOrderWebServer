...
public class ObligationExcelService : IObligationExcelService
{
    ...
    public async Task<byte[]> GenerateObligationAsync(VesselCallDto vesselCall)
    {
        ...
        var allContainers = vesselCall.BillOfLadings?
            .SelectMany(bl => bl.ContainerRecords.Where(s => !s.IsSoc), (bl, record) => new { ContainerNo = record.ContainerNo.Substring(0,11), IsoCode = $"{record.ContainerTypeId.Substring(2, 2)}{record.ContainerTypeId.Substring(0, 2)}", TareWt = record.TareWt, BillOfLading = bl.Num }).DistinctBy(s=>s.ContainerNo).ToList().OrderBy(c => c.ContainerNo);
