...
public class UploadFileService : IUploadFileService
{
    ...
    private readonly IExportOrderProvider_New ExportOrderProvider;
    private readonly ISupplementaryUnitProvider _supplementaryUnitProvider;

    public UploadFileService(ICntrTypeProvider cntrTypeProvider,
                                IDocumentProvider documentProvider,
                                IExportOrderProvider_New exportOrderProvider,
                                ISupplementaryUnitProvider supplementaryUnitProvider)
    {
        _cntrTypeProvider = cntrTypeProvider;
        _documentProvider = documentProvider;
        ExportOrderProvider = exportOrderProvider;
    ...
    public async Task<List<string>> CheckUploadedExportOrder(List<ReadExcelExportOrderRecordDTO> readResult, long eoId, long voyageId)
    {
        ...
        {
            using (IValidationService_New validationService = new ValidationService_New())
            {
                /// Коллекция существующих номеров кнтр-ов, исключая редактируемую коллекцию (edited)
                var dbVoyageCntrNums = await ExportOrderProvider.GetCntrNumsInVoyage(eoId, voyageId);

                var cntrNumErros = validationService.ValidateCntrNums(uploadedCntrNums!, dbVoyageCntrNums);

                if (!string.IsNullOrEmpty(cntrNumErros))
                    errList.Add(cntrNumErros.Insert(0, "---:"));
            }
        }
